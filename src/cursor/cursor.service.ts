...
const CLEANUP_TASK = [
  ...
  'Name all created branches using: `jarvis/fix-*`',
...
const UPDATE_CLEANUP_FALLBACK = [
  ...
  'Name branches using: `jarvis/fix-*`',
  'When you open a pull request, open it as ready for review (not draft).',
  'Create a PR targeting `main`. When checks pass, squash-merge into `main`, or enable GitHub auto-merge to `main` if the integration supports it.',
].join('\n');
...
/** Workspace chat updates: user text only + minimal safety lines (no Jarvis patch LLM). */
...
const FULL_BUILD_TASK = [
  ...
  'This app is generated by Jarvis, a natural-language AI app builder (Lovable / Bolt.new-style). The person who requested it is usually NOT a developer: they described an app in plain English and will judge the result by a live preview and a one-click deploy — not by reading the code. So the app must feel real and finished on the FIRST try:',
  ...
  'TYPOGRAPHY & BACKGROUNDS: the kit sets the global font (Apple SF Pro system stack via --font-sans) and refined heading tracking/weight. In src/index.css do NOT set a font-family and do NOT import a Google font (no Inter/Roboto) — just `@import "./styles/ui-kit.css";` then `@import "tailwindcss";`. For attractive, on-brand section backgrounds use the kit gradient utilities `.bg-gradient-mesh` (hero), `.bg-gradient-brand` (CTA bands), `.bg-gradient-subtle` (section tint), and `.text-gradient` (headline accent) instead of flat colours — keep the page body neutral.',
  '',
  ...
  'Name all created branches using: `jarvis/build-*`',
  'When you open a pull request, open it as ready for review (not draft).',
  'Create a PR targeting `main`. When checks pass, squash-merge into `main`, or enable GitHub auto-merge to `main` if the integration supports it.',
].join('\n');
...
function buildFullBuildPrompt(spec: string): string {
  ...
  return [FULL_BUILD_TASK, '', '---', 'DEVELOPMENT PLAN:', '', spec.trim()].join(
    '\n',
  );
}
...
export class CursorService {
  ...
  /**
   * Run Cursor Cloud Agent with the user's message only (no Jarvis patch LLM).
  ...
  async runStandaloneUserPromptRound(
    ...
    const repoHttps = `https://github.com/${owner}/${repo}`;
    const branch = `jarvis/chat-${projectId}-${Date.now()}`;
  ...
  async runFullBuildRound(input: FullBuildInput): Promise<CursorRoundResult> {
    ...
    const repoHttps = `https://github.com/${owner}/${repo}`;
    const branch = `jarvis/build-${projectId}-${Date.now()}`;
  ...
  async runRound(input: CursorRoundInput): Promise<CursorRoundResult> {
    ...
    const branch = `jarvis/fix-${revisionId}-${attempt}`;
    const repoHttps = `https://github.com/${owner}/${repo}`;
