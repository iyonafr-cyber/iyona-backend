...
- The HOME HERO is the Hero BLOCK at the variant fixed in the design system block above. Do not re-describe its layout — specify only its CONTENT: headline, accent word, subcopy, CTA labels and routes, stats, and which library image (if that variant takes one).
...
## 4. Global structure
Open with ONE line naming the product CATEGORY and the 2–3 reference sites whose conventions you followed (per LAYOUT GROUNDING above) — e.g. "Category: specialty coffee shop; references: Blue Bottle, Stumptown — hero with full-bleed product imagery, menu-first nav, story/about section, locations, newsletter footer." Then describe the PUBLIC shell. The header and footer are the SiteHeader / SiteFooter BLOCKS whose variants the design system block already fixed — do NOT design a header or footer layout, and do NOT prescribe one uniform look: specify only the CONTENT (nav items and their routes, the footer groups and links, the wordmark, the header action). Give the layout container width and the page scaffold each screen follows, and keep the section order aligned with the category conventions you named.
${ADMIN_FOR_PLAN}
