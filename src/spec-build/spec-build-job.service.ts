...
const USER_SAFE_GITHUB =
  'GitHub is not configured correctly on the server (invalid or expired token). Ask an admin to update GITHUB_PAT / JARVIS_GITHUB_TOKEN.';
const USER_SAFE_CURSOR_GITHUB =
  'Cursor cannot access the project GitHub repo. Install the Cursor GitHub App on the org that owns repos (JARVIS_GITHUB_ORG) in Cursor → Integrations, or point JARVIS_GITHUB_ORG at an org Cursor already covers.';
  ...
  async processJob(jobId: string): Promise<void> {
    ...
    try {
      await this.repoService.runExclusive(projectId, async () => {
        ...
        const freshAnswers = (jobFresh.answers ?? {}) as Record<string, unknown>;
