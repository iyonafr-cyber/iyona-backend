...
export class SupabaseSchemaService {
   ...
   */
  private normalizeSchema(schema: IyonaSchemaDeclaration): void {
    for (const table of schema.tables) {
   ...
   * database connection string, Iyona cannot apply DDL, so it hands the owner
   ...
   */
  buildSchemaSql(schema: IyonaSchemaDeclaration): string {
    this.normalizeSchema(schema);

    const blocks: string[] = [
      '-- Generated by Iyona. Safe to run more than once.',
   ...
   */
  private buildTableSql(table: IyonaTableDef): string {
...
`.trim();
   ...
   */
  private buildColumnDef(col: IyonaColumnDef): string {
   ...
   */
  private buildPolicySql(tableName: string, policy: IyonaRlsPolicy): string {
